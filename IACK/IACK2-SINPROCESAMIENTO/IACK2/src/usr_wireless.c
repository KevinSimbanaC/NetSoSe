	...
	// This code block will be called only if the transmission is enabled.
		...
		if(coordinador){
			if(contTramas < 2 && transmitir == true){//Solo si es coordinador puede transmitir
				...
				transmitir = false;
				contTramas++;
				delay_ms(4);//Retardo para procesar la trama que llega y enviar la siguiente trama
			}		
		}
...
void usr_frame_received_cb(frame_info_t *frame)
{
		...
		if(coordinador){
			...
			transmitir = true;// Para habilitar la transmision
			tipoTx = false; // Para enviar el segundo mensaje
			
			//************************************************//
			 /********* SE ELIMINA EL PROCESAMIENTO *********/
			//***********************************************//
